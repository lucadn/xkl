...
void spectgrid(char *name,int pagenumber,int freqmax,char *date,char *action){
	...
	post_text(90, action,0,0,0,0,0,0,0,0);
...
void post_f0_grid(char *name,int pagenumber, int freqmax,char * date,char *action)
{
...
void post_f0_plot(f0, ampl, time_ms) int f0,ampl,time_ms; {
...
void post_wave_plot(short *iwave,int xpos_of_wave0,int nwpts){
...
void post_wave_axis(char *name,int xpos_of_wave0,int nwpts,char *date,int nlpp,char *action){
...
*/
	...
	post_text(90, frstname,0,0,0,0,0,0,0,0);
...
void post_grid(char *name,int pagenumber,int freqmax,char *date,char flabel){
 int box_wid,box_hyt;
 extern char frstname[];
...
/*    Raise case and then print path name */
	...
	post_text(90, label,0,0,0,0,0,0,0,0);		/* Rotate 90 degrees */
...
/*    Label frequency axis */
	if (flabel == '0') {
	    ...
	    post_text(180, "F0  (HZ)",0,0,0,0,0,0,0,0);
	}
	else {
	    ...
	    post_text(180, "FREQ  (kHz)",0,0,0,0,0,0,0,0);
	}
...
void post_amp_axis(int xlocbottom,int pagenumber) {
 int box_wid,box_hyt;
...
/*    Label amplitude axis */
	...
	post_text(180, "AMP  (dB)",0,0,0,0,0,0,0,0);
...
void leftlabel(value,x,y) int value,x,y; {
...
void bottomtick(y,time) int time,y; {

	post_point(xminpost+5,y);
	post_cont(xminpost+10,y);
	post_move(xminpost+30,y-18);
	sprintf(label, "%4d", time);
	if (y+15 < yposmax)	post_text(90, label,0,0,0,0,0,0,0,0);
}


void toptick(y,time) int time,y; {
...
void post_spec_plot(inspec, ampdb, time_ms)
...
void post_freq_plot(infreq, ninfreq, ampdb, time_ms)
